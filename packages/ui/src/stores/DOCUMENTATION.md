...
Context-panel persistence v22 canonicalizes directory keys from snapshots through v21 with the same helper used by readers and mutations. Historical keys that collapse to one directory are merged during hydration: the most recently touched state owns open/active presentation, while tabs and per-surface widths are retained across colliding entries. The older preview-to-browser migration runs first; upstream terminal targets, per-runtime Linear filters, and explicit telemetry preferences remain intact.

Colliding panel widths preserve both pixels and responsive fractions. A newer pixel width without a fraction clears that mode's older fraction, matching the normal pixel-only resize behavior. Widths for other modes remain intact.
