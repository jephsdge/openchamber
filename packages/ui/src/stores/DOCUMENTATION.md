...
Context-panel persistence canonicalizes directory keys with the same helper used by every reader and mutation. Historical keys that collapse to one directory are merged during hydration: the most recently touched state owns open/active presentation, while tabs and per-surface widths are retained across colliding entries.
